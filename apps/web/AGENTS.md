...
- OpenAI is only reached from `server/*.mjs` behind the session budget cap; every shop-drawer search (address bar
  and ★ agent) costs ~1¢ per call (web search tool), imports ~0.1¢. Test them sparingly; unit tests never call the API.
