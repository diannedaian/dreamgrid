...
import { applySwap, fitToBudget, rankAlternatives, revertSwaps, type Alternative } from "./alternatives";
...
export function mountBudgetBar(bar: HTMLElement, chip: HTMLButtonElement, o: BudgetBarOptions): BudgetBar {
  let budgetUsd = Math.max(0, Math.round(o.budgetUsd));
  ...
  const products = (): Product[] => o.catalog.entries().map((e) => e.product);
  ...
  const applyState = async (next: RoomState) => {
    ...
    try {
      for (const item of next.items) {
        ...
        if (product) await o.swap(item.id, product);
      }
  ...
  const chipLabel = (s: BudgetSummary) => {
    ...
    if (budgetUsd <= 0) { chip.textContent = "Budget"; return; }
    chip.classList.add(s.status === "over" ? "over" : "under");
    chip.textContent = s.status === "over" ? `Budget · ${formatUsd(-s.remainingUsd)} over` : `Budget · ${formatUsd(s.remainingUsd)} left`;
  };

  // ── main pane ──────────────────────────────────────────────────────────────
  const renderBudget = () => {
    const s = summarize();
    const alts = rankAlternatives(state(), products());
    ...
    pane.innerHTML = `
      <label class="amount-row"><span>My budget</span><span class="cur">$</span><input class="amount" type="number" min="0" step="1" value="${budgetUsd || ""}" placeholder="0" /></label>
      <div class="totals">
        <div><span>Room total</span><b class="subtotal">${formatUsd(s.subtotalUsd)}</b></div>
      ...
      <p class="status ${tone}">${budgetUsd > 0 ? STATUS_LABEL[s.status] : "Type a budget to see how this room measures up."}</p>
      ...
      ${s.lines.length ? `<ul class="lines">${s.lines.map((l) => `<li><span>${esc(l.product.title)}</span><span class="dots"></span><b>${formatUsd(l.product.priceUsd)}</b></li>`).join("")}</ul>` : `<p class="empty">Nothing placed yet. Drag furniture in from the catalog and it shows up here with its price.</p>`}
      ...
      <div class="agent-actions"><button type="button" class="find review" ${s.lines.length === 0 ? "disabled" : ""}>Review shopping plan</button></div>`;

    const amount = pane.querySelector<HTMLInputElement>(".amount")!;
    amount.addEventListener("change", () => {
      budgetUsd = Math.max(0, Math.round(Number(amount.value) || 0));
      fitMessage = "";
    ...
    pane.querySelector(".fit")?.addEventListener("click", async () => {
      const r = fitToBudget(state(), products());
      swapsApplied = [...swapsApplied, ...r.swaps];
      fitMessage = r.fitsBudget
        ? `Applied ${r.swaps.length} swap(s); ${formatSignedUsd(r.remainingUsd)} remaining.`
        : `Applied ${r.swaps.length} swap(s) but the room is still ${formatSignedUsd(r.remainingUsd)}; no more cheaper options.`;
      await applyState(r.state);
    });
    pane.querySelector(".undo")?.addEventListener("click", async () => {
      const next = revertSwaps(state(), swapsApplied);
      swapsApplied = []; fitMessage = "";
    ...
    pane.querySelectorAll<HTMLButtonElement>(".apply").forEach((btn) => btn.addEventListener("click", async () => {
      ...
      if (!a) return;
      swapsApplied = [...swapsApplied, a]; fitMessage = "";
      await applyState(applySwap(state(), a));
    }));
  ...
  const renderPlan = (p: ShoppingPlan) => {
    ...
    pane.innerHTML = `
      ...
      ${approved
        ? `<p class="sub">Your plan is saved. Open each store link above to buy the items yourself; DreamGrid does not place orders.</p>
  ...
  return {
    refresh: () => {
      ...
      if (plan && plan.status !== "approved") plan = undefined;
      render();
    },
    setOpen,
