...
function priceFit(price: number | undefined, maxPriceUsd: number | undefined): number {
  if (price === undefined) return 0.3; // unknown: neither rewarded nor thrown out
  if (maxPriceUsd === undefined || maxPriceUsd <= 0) return 0.7;
...
export function rankSearchResults(
  ...
  const ranked = results.map((draft): RankedResult => {
    const price = priceFit(draft.priceUsd, query.maxPriceUsd);
    const size =
    ...
    const overBudgetUsd =
      draft.priceUsd !== undefined && query.maxPriceUsd !== undefined
        ? Math.max(0, Math.round((draft.priceUsd - query.maxPriceUsd) * 100) / 100)
        : 0;

    const reasons: string[] = [];
    if (draft.priceUsd === undefined) reasons.push("Price unknown");
    else if (overBudgetUsd > 0) reasons.push(`$${overBudgetUsd} over your limit`);
    else if (query.maxPriceUsd !== undefined) reasons.push("Within your limit");
