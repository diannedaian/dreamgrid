...
describe("buildProduct", () => {
  ...
  it("uses defaults for merchant and source when entered by hand", () => {
    ...
    expect(product.sourceUrl).toBe("https://example.com/manual-entry");
  });

  it("refuses a product the shared schema rejects", () => {
    expect(() => buildProduct({ ...filled, title: "   " }, "p")).toThrow(/Product validation failed/);
  });
});
