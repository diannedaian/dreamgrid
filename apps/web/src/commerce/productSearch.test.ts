...
describe("rankSearchResults", () => {
  it("does not advertise zero placeholders as free or treat a zero limit as unlimited", () => {
    const ranked = rankSearchResults({ category: "desk", maxPriceUsd: 0 }, [draft({ sourceUrl: "zero", priceUsd: 0 }), draft({ sourceUrl: "priced", priceUsd: 20 })]);
    expect(ranked.find(r => r.draft.sourceUrl === "zero")?.reasons).toEqual([]);
    expect(ranked.find(r => r.draft.sourceUrl === "priced")?.overBudgetUsd).toBe(20);
  });
  ...
  it("scores neutrally when the query has no target size or style", () => {
    ...
    expect(ranked.find((r) => r.draft.title === "Unknown price")?.reasons).toEqual([]);
  });
});
