...
@pytest.mark.anyio
async def test_search_recovers_missing_photos_without_ai_or_price_changes() -> None:
    url = "https://shop.example/chair"
    draft = ProductDraft(source_url=url, title="Search title", missing=("imageUrl", "priceUsd"))
    fetcher = FakeFetcher({url: PRODUCT_PAGE})
    model = FakeModel(OpenAIError("No paid extraction allowed"))
    search = FakeSearch(SearchOutcome(results=(draft,), source="live", provider="openai"))
    service = ProductSourcingService(fetcher, LlmProductExtractor(model), search)
    result = await service.search(ProductQuery(category="chair"))
    assert result.results[0].image_url == "https://cdn.example/chair.jpg"
    assert result.results[0].title == "Search title"
    assert result.results[0].price_usd is None
    assert result.results[0].missing == ("priceUsd",)
    assert model.calls == []
    await service.search(ProductQuery(category="chair"))
    assert fetcher.calls == [url]


@pytest.mark.anyio
async def test_search_keeps_blocked_results_and_skips_photos_already_provided() -> None:
    ready = ProductDraft(
        source_url="https://shop.example/ready", image_url="https://cdn.example/x.jpg"
    )
    blocked = ProductDraft(source_url="https://shop.example/blocked")
    fetcher = FakeFetcher({}, fail={blocked.source_url: "HTTP 403"})
    search = FakeSearch(SearchOutcome(results=(ready, blocked), source="live", provider="serpapi"))
    service = ProductSourcingService(fetcher, NullProductExtractor(), search)
    result = await service.search(ProductQuery(category="chair"))
    assert result.results == (ready, blocked)
    assert fetcher.calls == [blocked.source_url]


@pytest.mark.anyio
async def test_fixture_search_never_fetches_listing_photos() -> None:
    draft = ProductDraft(source_url="https://shop.example/chair")
    fetcher = FakeFetcher({})
    search = FakeSearch(SearchOutcome(results=(draft,), source="fixture"))
    result = await ProductSourcingService(fetcher, NullProductExtractor(), search).search(
        ProductQuery(category="chair")
    )
    assert result.results == (draft,)
...
async def test_service_caches_non_empty_search_results() -> None:
    ...
    service = ProductSourcingService(
        FakeFetcher({"https://a.example": "<html></html>"}), NullProductExtractor(), search
    )
...
def test_factory_defaults_to_fixture_components_without_a_key() -> None:
    settings = Settings(
        environment="test", product_sourcing="auto", openai_api_key="", serpapi_api_key=None
    )
