...
from dreamgrid_api.adapters.commerce.page_fetcher import PageFetchError, fetch_listing_image
...
class SearchProductsRequest(CamelModel):
    ...
    target_dimensions_m: (
        tuple[
            Annotated[float, Field(gt=0, le=100, allow_inf_nan=False)],
            Annotated[float, Field(gt=0, le=100, allow_inf_nan=False)],
            Annotated[float, Field(gt=0, le=100, allow_inf_nan=False)],
        ]
        | None
    ) = None
    max_price_usd: float | None = Field(default=None, ge=0, allow_inf_nan=False)
...
@router.get("/photo")
async def listing_photo(url: Annotated[str, Query(min_length=1, max_length=2048)]) -> Response:
    """Proxy one listing photo so the browser can seed the generation panel with it."""

    try:
        image = await fetch_listing_image(url)
