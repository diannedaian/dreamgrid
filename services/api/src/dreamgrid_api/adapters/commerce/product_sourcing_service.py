...
class ProductSourcingService:
    ...
    def __init__(
        ...
        self._listing_cache: _LruCache[str, ProductDraft] = _LruCache()
        self._photo_slots = asyncio.Semaphore(4)
    ...
    async def import_from_url(self, url: str, *, title_hint: str | None = None) -> ProductDraft:
        ...
        self._listing_cache.put(key, draft)
    ...
    async def search(self, query: ProductQuery, *, limit: int = 8) -> SearchOutcome:
        ...
        if outcome.source == "live":
            photos = await asyncio.gather(*(self._with_photo(draft) for draft in outcome.results))
            outcome = replace(outcome, results=tuple(photos))
        if outcome.results:
            self._search_cache.put(key, outcome)
        return outcome

    async def _with_photo(self, draft: ProductDraft) -> ProductDraft:
        if draft.image_url:
            return draft
        async with self._photo_slots:
            listing = self._listing_cache.get(draft.source_url)
            if listing is None:
                try:
                    async with asyncio.timeout(10):
                        page = await self._fetcher.fetch(draft.source_url)
                        listing = parse_product_page(page.html, page.final_url or draft.source_url)
                except (PageFetchError, TimeoutError, ValueError):
                    return draft
                self._listing_cache.put(draft.source_url, listing)
        if not listing.image_url:
            return draft
        return replace(
            draft,
            image_url=listing.image_url,
            missing=tuple(field for field in draft.missing if field != "imageUrl"),
        )
...
def build_product_sourcing(settings: Settings) -> ProductSourcingService:
    ...
    if settings.sourcing_is_live and settings.openai_api_key.get_secret_value():
        model = OpenAIResponsesModel(
            settings.openai_api_key.get_secret_value(), settings.commerce_openai_model
        )
